...
public class UserServiceImpl implements IUserService ,ApplicationContextAware{
    ...
    public ServerResponse<User> login(String username, String password) {
        ...
        if (user == null) {
            return ServerResponse.createByErrorMessage("该用户密码错误");
        }
    ...
    public ServerResponse<String> selectQuestion(String username) {
        ...
        if (!StringUtils.isNotBlank(question)) {
            ServerResponse.createByErrorMessage("找回密码问题为空");
        }
    ...
    public ServerResponse<String> checkAnswer(String username, String question, String answer) {
        ...
        if (resultCount > 0) {
            //说明该用户的问题答案都是正确的
            //需要一个token，修改密码的时候需要用这个。用来传递给修改密码接口
            String forgetToken = UUID.randomUUID().toString();
            //TokenCache.setKey(TokenCache.TOKEN_PREFIX + username, forgetToken);
            //改造成redis存取
            RedisShardedPoolUtil.setEx(Const.TOKEN_PREFIX + username,forgetToken,60*60*12);//存12小时
    ...
    public ServerResponse<String> forgetResetPassword(String username, String passwordNew, String forgetToken) {
        if (StringUtils.isBlank(forgetToken)) {
            return ServerResponse.createByErrorMessage("参数错误，需要传递forget token");
        }
...
//        String token = TokenCache.getKey(TokenCache.TOKEN_PREFIX + username);
        ...
        if (StringUtils.equals(token, forgetToken)) {
            ...
            return ServerResponse.createByErrorMessage("token错误，请重新申请密码找回");
        }
    ...
    public ServerResponse<User> updateInformation(User user) {
        ...
        if(StringUtils.isNotBlank(user.getEmail())){
            ...
            return ServerResponse.createByErrorMessage("email不能为空，请填写");
        }
